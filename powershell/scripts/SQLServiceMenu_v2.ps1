...
function Show-Menu {
    ...
    Write-Host "3. Alternar estado (On/Off) y salir"
    Write-Host "4. Salir"
    Write-Host "====================================="
}
...
function Toggle-SQL {
    ...
    Write-Host "`n👋 Acción completada, saliendo..."
    exit
}
...
do {
    ...
    switch ($choice) {
        "1" { Start-SQL }
        "2" { Stop-SQL }
        "3" { Toggle-SQL }   # Alterna y sale automáticamente
        "4" { Write-Host "👋 Saliendo..."; break }
        default { Write-Host "⚠ Opción inválida" }
    }

    if ($choice -ne "3" -and $choice -ne "4") {
        Write-Host "`nÚltimas acciones registradas:"
